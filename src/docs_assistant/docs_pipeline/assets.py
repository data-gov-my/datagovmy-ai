    ...
    for commit in commits:
        ...
        if commit_date > last_check:
            ...
            for file in commit_data['files']:
                ...
                if file['filename'].startswith(path_to_folder) and file['filename'].endswith('.mdx'):
...
@multi_asset(
    outs={"mdx_recs_to_update": AssetOut(is_required=False), 
...
def mdx_vect_update(mdx_recs_to_update: pd.DataFrame) -> None:
    ...
    vect = WeaviateVectorDB(meta_fields=mdx_fields, 
